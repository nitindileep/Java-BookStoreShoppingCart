				...
				<c:forEach items="${cart.cartItems}" var="cartItem" varStatus="loop">
					<tr>
						<form action="/cart/update" name="cart_form">
							<input name="index" type="hidden" value="<c:out value='${ loop.index }' />" />
							<td>${ cartItem.getTitle() }</td>
							<td>${ cartItem.getAuthor() }</td>
							<td>
								<fmt:formatNumber value="${ cartItem.getPrice() }" type="currency" />
							</td>
							<td>
							...
							<td>
								<fmt:formatNumber value="${ cartItem.getTotalCost() }" type="currency" />
							</td>
							<td>
